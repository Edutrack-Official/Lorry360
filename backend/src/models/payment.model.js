...
const PAYMENT_TYPE_ENUM = ['to_crusher', 'from_customer', 'to_collab_owner', 'to_bunk'];
...
const paymentSchema = new mongoose.Schema({
  ...
  // For collaboration payments - the other owner receiving payment
  collab_owner_id: {
    type: mongoose.Schema.Types.ObjectId,
    ref: 'User',
    required: function() {
      return this.payment_type === 'to_collab_owner';
    }
  },
  ...
  bunk_id: {
    ...
    ref: 'PetrolBunk',
    required: function() {
      return this.payment_type === 'to_bunk';
    }
  },
  ...
  // For tracking collaboration payment status
...
paymentSchema.index({ owner_id: 1, payment_type: 1 });
paymentSchema.index({ bunk_id: 1, isActive: 1 });
paymentSchema.index({ crusher_id: 1, isActive: 1 });
paymentSchema.index({ customer_id: 1, isActive: 1 });
paymentSchema.index({ collab_owner_id: 1, isActive: 1 });
