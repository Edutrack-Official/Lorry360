...
import {
  ...
  RefreshCw,
  ChevronDown,
  DollarSign, // Add this icon
  TrendingDown, // Add this icon
...
// NEW: Price Change Modal Component
...
const PriceChangeModal: React.FC<PriceChangeModalProps> = ({
  ...
  // Calculate date range for selected trips
  const getDateRange = () => {
  ...
  // Group trips by material for summary
        ...
        <div className="relative w-full max-w-2xl bg-white rounded-xl shadow-2xl">
          ...
          <div className="px-6 py-5 max-h-[60vh] overflow-y-auto">
            <div className="space-y-6">
              ...
              {/* Extra Amount Input (only shown when updateCustomerAmount is true) */}
...
const LorryTrips = () => {
  ...
  const [selectedTrips, setSelectedTrips] = useState<Array<{ id: string; tripNumber: string }>>([]);
  const [cloneModalOpen, setCloneModalOpen] = useState(false);
  const [priceChangeModalOpen, setPriceChangeModalOpen] = useState(false); // NEW
  const [showBulkActions, setShowBulkActions] = useState(false);
  let refersh = false;
                  ...
                  <AnimatePresence>
                    {showBulkActions && (
                      <>
                        ...
                        {/* Dropdown Menu - UPDATED WITH PRICE CHANGE OPTION */}
                        <motion.div
                          initial={{ opacity: 0, scale: 0.95, y: -10 }}
                          animate={{ opacity: 1, scale: 1, y: 0 }}
                          exit={{ opacity: 0, scale: 0.95, y: -10 }}
