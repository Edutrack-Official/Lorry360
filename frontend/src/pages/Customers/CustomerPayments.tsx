...
const CustomerPayments = () => {
  ...
  // Helper function to normalize amount for searching
  const normalizeAmountForSearch = (amount: number): string => {
    return amount.toString().replace(/,/g, '');
  };

  // Helper function to check if search text matches amount
  const matchesAmountSearch = (amount: number, searchText: string): boolean => {
    ...
    // Check for currency formatted amount
  ...
  const filteredPayments = payments.filter((payment) => {
    // Search filter - Enhanced for all fields
    const matchesSearch =
      searchText === "" ||
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-8 sm:p-12 text-center">
            ...
            {payments.length === 0 ? (
              <p className="text-sm text-gray-600 mb-6 max-w-md mx-auto">
                Start by recording your first payment from this customer
              </p>
            ) : searchText ? (
        ...
        {showActionMenu && (
          <>
            ...
            >
              <button
                onClick={() => {
                  // Proper navigation to edit page with customerId
                  navigate(`/customers/${customerId}/payments/edit/${showActionMenu}`);
                  setShowActionMenu(null);
                }}
