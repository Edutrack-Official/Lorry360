    ...
    return (
      ...
      >
        <div className="flex items-start justify-between mb-3">
          ...
          <div className="text-right">
            <p className="font-bold text-gray-900 text-base">{formatCurrency(trip.customer_amount)}</p>
          </div>
        </div>
        ...
        <div className="space-y-2 text-xs mb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="truncate text-gray-700">{trip.material_name}</span>
          </div>
          <div className="flex items-center gap-2">
            <MapPin className="h-4 w-4 text-gray-400 flex-shrink-0" />
            <span className="truncate text-gray-700">{trip.location}</span>
          </div>
        </div>

        {trip.notes && (
          <p className="text-xs text-gray-600 bg-gray-50 rounded-lg p-2 mb-3">{trip.notes}</p>
        )}
        ...
        {canApproveReject && trip.collab_trip_status === 'pending' && (
          <div className="flex gap-2 pt-3 border-t border-gray-100">
            <button
              onClick={() => handleApproveTripStatus(trip._id, trip.collab_owner_id?._id)}
              className="flex-1 inline-flex items-center justify-center gap-1.5 px-4 py-2.5 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm font-medium active:scale-95 transition-transform"
            >
            ...
            <button
              onClick={() => handleRejectTripStatus(trip._id, trip.collab_owner_id?._id)}
              className="flex-1 inline-flex items-center justify-center gap-1.5 px-4 py-2.5 bg-red-600 text-white rounded-lg hover:bg-red-700 text-sm font-medium active:scale-95 transition-transform"
            >
      ...
      >
        <div className="flex items-start justify-between mb-3">
          <div className="flex-1">
            <h4 className="font-bold text-gray-900 text-sm mb-2">{payment.payment_number}</h4>
            <div className="flex flex-wrap items-center gap-2">
              <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg text-xs font-medium border ${modeConfig.color}`}>
        ...
        {payment.notes && (
          <p className="text-xs text-gray-600 bg-gray-50 rounded-lg p-2 mb-3">{payment.notes}</p>
        )}
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-20 sm:pb-4">
      ...
      <div className="bg-white border-b shadow-sm sticky top-0 z-10">
        <div className="px-4 py-3 sm:py-4">
            ...
            <div className="flex items-center gap-2 sm:gap-3 flex-1 min-w-0">
              ...
              <div className="flex items-center gap-2 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  <h1 className="text-base sm:text-lg font-bold text-gray-900 truncate">{partner.name}</h1>
                  <p className="text-xs sm:text-sm text-gray-600 truncate">{partner.company_name}</p>
                </div>
              </div>
            </div>
            <button
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-3">
            ...
            <div className="bg-gradient-to-br from-green-50 to-green-100 p-3 rounded-lg border border-green-200">
              <p className="text-xs text-green-700 mb-1">Total Payments (Approved)</p>
              <p className="text-base sm:text-lg font-bold text-green-900">{formatCurrency(totals.totalPaymentsMade)}</p>
              <p className="text-xs text-green-600">
                Mine: {formatCurrency(totals.totalMyPayments)} • Partner: {formatCurrency(totals.totalPartnerPayments)}
              </p>
            </div>
      ...
      <div className="px-4 py-4">
        ...
        <div className="bg-white rounded-lg border shadow-sm p-3 mb-4">
          <div className="flex overflow-x-auto scrollbar-hide mb-3 -mx-3 px-3">
            {activeMainTab === 'trips' ? (
              <>
                <button
                  onClick={() => setActiveTripsTab('my-trips')}
            ...
            {showFilters && (
              ...
              >
                <div className="pt-3 space-y-3">
                  ...
                  {/* Clear Filters */}
                  {(searchText || dateRange.start || dateRange.end || tripStatusFilter !== 'all' || paymentStatusFilter !== 'all') && (
                    <button
                      onClick={() => {
                        setSearchText('');
                      ...
                      className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200"
                    >
                      <X className="h-4 w-4" />
                      Clear Filters
                    </button>
        ...
        {/* Add Payment Button - Mobile Optimized */}
        {activeMainTab === 'payments' && activePaymentsTab === 'my-payments' && (
          <div className="mb-4">
            <button
              onClick={() => navigate(`/partners/${partnerId}/payments/create`)}
              className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm font-medium shadow-md active:scale-95 transition-transform"
            >
              <Plus className="h-4 w-4" />
              Add Payment
            </button>
        ...
        <div className="bg-white rounded-lg border shadow-sm p-4">
          ...
          {activeMainTab === 'trips' ? (
            filteredTrips().length === 0 ? (
              <div className="text-center py-12">
                <Truck className="h-12 w-12 mx-auto mb-3 text-gray-300" />
                <h4 className="text-sm sm:text-base font-semibold text-gray-900 mb-2">No completed trips found</h4>
                <p className="text-xs sm:text-sm text-gray-600">Trips must be marked as "completed" to appear here for approval</p>
              </div>
            ) : (
            ...
            filteredPayments().length === 0 ? (
              <div className="text-center py-12">
                <CreditCard className="h-12 w-12 mx-auto mb-3 text-gray-300" />
                <h4 className="text-sm font-semibold text-gray-900 mb-2">No payments found</h4>
              </div>
            ) : (
              <div className="space-y-3">
                {filteredPayments().map((payment) => (
