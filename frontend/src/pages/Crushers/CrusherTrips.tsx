...
import { Plus, Package, Calendar, IndianRupee, MapPin, Truck, User } from 'lucide-react';
...
const CrusherTrips = () => {
  ...
  const fetchCrusherTrips = async () => {
    ...
    try {
      const res = await api.get(`/trips?crusher_id=${crusherId}`);
    ...
    <div className="space-y-6">
      ...
      {trips.length > 0 ? (
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
                    ...
                    {trip.customer_id && (
                      <div className="flex items-center gap-2">
                        ...
                        <span>{trip.customer_id.name}</span>
                      </div>
                    )}
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-500 mb-4">Start by adding your first trip for this crusher</p>
          <Link
            to={`/trips/create?crusher=${crusherId}`}
            className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
          >
