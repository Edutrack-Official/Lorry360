    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
      ...
      >
        ...
        <div className="p-6 space-y-6">
          ...
          {/* Date Suggestion Alert - COMPACT & CRISP */}
          {showDateSuggestion && suggestedDates && (
            <motion.div
              initial={{ opacity: 0, y: -10 }}
              animate={{ opacity: 1, y: 0 }}
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 mb-1">
                    ...
                    {suggestedDates.priority === 'critical' && (
                      <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-red-200 text-red-800">
                        CRITICAL
                      </span>
                    )}
                    {suggestedDates.trip_count > 0 && (
                      <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-green-200 text-green-800">
                  ...
                  {suggestedDates.alternativeRanges && suggestedDates.alternativeRanges.length > 0 && (
                    <div className="mt-2 pt-2 border-t border-blue-200">
                      ...
                      <div className="space-y-0.5">
                        {suggestedDates.alternativeRanges.slice(0, 2).map((range, index) => (
                          <div key={index} className="flex items-center justify-between text-[10px] text-blue-600">
                            <span>
                              {formatDate(range.from_date)} → {formatDate(range.to_date)}
                            </span>
                            <span className="font-medium">({range.trip_count})</span>
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <button
                  onClick={handleUseSuggestion}
                  className={`px-3 py-1.5 text-white rounded-lg transition-colors text-xs font-medium whitespace-nowrap self-start ${
                    suggestedDates.priority === 'critical'
                      ? 'bg-red-600 hover:bg-red-700'
                      : 'bg-blue-600 hover:bg-blue-700'
                  }`}
