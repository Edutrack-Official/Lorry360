...
//     if (!window.confirm(`Delete reminder "${reminder.note}"?`)) return;
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b sticky top-0 z-10 shadow-sm">
        <div className="px-4 py-4 sm:px-6">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              {/* Only show toggle if not locked by WhatsApp */}
              {!reminder.whatsappSent && (
                <button
                  onClick={handleToggleStatus}
                  disabled={updating || reminder.done}
                  className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                    reminder.done
                      ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                      : 'bg-blue-100 text-blue-700 hover:bg-blue-200'
                  }`}
                  title={reminder.done ? "Cannot mark as pending once done" : "Mark as done"}
                >
                  {updating ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                  ) : (
      ...
      <div className="p-4 sm:p-6 max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden mb-6">
          <div className="p-4 sm:p-6">
            ...
            <div className="space-y-4 mb-6">
              ...
              {reminder.sendWhatsapp && reminder.whatsappSent && reminder.whatsappSentAt && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                  ...
                  <p className="text-sm text-green-900">
                    Sent on {formatDateTime(reminder.whatsappSentAt)}
                  </p>
                </div>
              )}
        ...
        {!reminder.done && !reminder.whatsappSent ? (
          <div className="sticky bottom-0 bg-white border-t border-gray-200 -mx-4 sm:-mx-6 px-4 sm:px-6 py-4 sm:static sm:border-0 sm:px-0 sm:py-0 sm:bg-transparent shadow-lg sm:shadow-none">
            <div className="flex flex-col sm:flex-row gap-3">
              <button
                onClick={() => navigate(`/reminders/edit/${reminder._id}`)}
          ...
          <div className="bg-gray-50 rounded-lg border border-gray-200 p-4 text-center">
            ...
            <p className="text-xs text-gray-500 mt-1">
              {reminder.whatsappSent ? 'WhatsApp notification has been sent' : 'Reminder has been marked as done'}
            </p>
          </div>
        )}
      </div>
