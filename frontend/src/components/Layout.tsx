...
const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      const response = await api.get(`/users/${authUser.userId}`);
  ...
  const filteredNavigation = navigation.filter((item) =>
    item.roles.includes(authUser?.role || "admin")
  );
  ...
  // Helper function to handle logo errors
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/30 to-gray-50">
          ...
          <div className="flex items-center gap-3 p-3 bg-white/10 rounded-xl backdrop-blur-sm">
            {logoUrl ? (
              <img
                src={logoUrl}
                alt="Profile"
                className="w-12 h-12 rounded-full object-cover border-2 border-white/30 shadow-lg flex-shrink-0"
                onError={handleLogoError}
              />
            ) : (
              <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-400 to-blue-600 flex items-center justify-center border-2 border-white/30 shadow-lg flex-shrink-0">
                <span className="text-white font-bold text-lg">
                  {(authUser?.name || userProfile?.name || "U").charAt(0).toUpperCase()}
                </span>
              </div>
            )}
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-white truncate capitalize">
                {authUser?.name || userProfile?.name || "User"}
              </p>
              <p className="text-xs text-blue-200 truncate capitalize">
                {authUser?.role || userProfile?.role || "user"}
              </p>
            </div>
          </div>
        </div>
      ...
      <aside className="hidden lg:fixed lg:inset-y-0 lg:flex lg:z-40">
        <div className="flex h-screen w-20 flex-col items-center bg-gradient-to-b from-blue-600 to-blue-700 shadow-2xl">
          {/* Logo */}
          <div className="flex h-16 w-16 items-center justify-center mt-4 mb-4 flex-shrink-0">
          ...
          <nav className="flex-1 py-2 flex flex-col items-center gap-2 w-full px-3 overflow-y-auto overflow-x-hidden min-h-0">
            {filteredNavigation.map((item) => {
              const Icon = item.icon;
              const isActive = location.pathname === item.href;
              return (
              ...
              <div className="relative" ref={userMenuRef}>
                ...
                >
                  {logoUrl ? (
                    ...
                    <div className="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center border-2 border-gray-200 shadow-sm flex-shrink-0">
                    ...
                    <div className="flex items-center gap-3 p-4 border-b border-gray-100">
                      {logoUrl ? (
                        <img
                          ...
                          alt="Profile"
                          onError={handleLogoError}
                          className="w-12 h-12 rounded-full object-cover border-2 border-blue-100 shadow-sm flex-shrink-0"
                        />
                      ) : (
                        <div className="w-12 h-12 rounded-full bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center border-2 border-blue-100 shadow-sm flex-shrink-0">
                          <span className="text-white font-bold text-lg">
                            {(authUser?.name || userProfile?.name || "U").charAt(0).toUpperCase()}
                          </span>
                        </div>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-500 truncate">
                          {authUser?.email || userProfile?.email || "user@example.com"}
                        </p>
                      </div>
                    </div>
